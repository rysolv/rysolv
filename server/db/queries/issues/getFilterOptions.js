...
const getFilterOptions = async () => {
  const queryText = `
    ...
    (SELECT COALESCE(ARRAY_AGG(DISTINCT(language)), '{}') FROM languages WHERE repo_id IS NOT NULL) AS "organizationLanguages",
    ...
    (SELECT COALESCE(MAX(funded_amount),0) FROM (SELECT repo_id, SUM(funded_amount) as funded_amount FROM payments GROUP BY repo_id) AS sub) AS "maxOrgFunded",
